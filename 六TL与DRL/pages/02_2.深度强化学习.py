...
with tab1:
    ...
    with col1:
        st.image('./六TL与DRL/图片14.png', caption='图14 ')
    """- 马尔可夫决策过程:"""
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./六TL与DRL/图片15.png', caption='图15 ')
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./六TL与DRL/图片16.png', caption='图16 ')
    """$ \qquad $ 马尔可夫过程:"""
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./六TL与DRL/图片17.png', caption='图17 ')
    """- 策略$\phi(a|s)$:"""
    """$ \qquad $ 马尔可夫决策过程的一个轨迹（trajectory）:"""
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./六TL与DRL/图片18.png', caption='图18 ')
    """$ \qquad $ τ的概率:"""
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./六TL与DRL/图片19.png', caption='图19 ')
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./六TL与DRL/图片20.png', caption='图20 ')
    """- 总回报:"""
    """$ \qquad $ 给定策略π(a|s)，智能体和环境一次交互过程的轨迹τ 所收到的累积奖励为总回报（return）"""
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./六TL与DRL/图片21.png', caption='图21 ')
    ...
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./六TL与DRL/图片22.png', caption='图22 ')
    """$ \qquad $ $\\theta$为策略函数的参数。"""
    """- 强化学习的算法:"""
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./六TL与DRL/图片23.png', caption='图23 ')
    """- 深度强化学习:"""
    ...
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./六TL与DRL/图片24.png', caption='图24 ')
    """$ \qquad $ 值函数：从状态s开始，执行策略π得到的期望总回报"""
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./六TL与DRL/图片25.png', caption='图25 ')
    """- Bellman方程:"""
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./六TL与DRL/图片26.png', caption='图26 ')
    """- 状态-动作值函数（ Q函数）:"""
    """$ \qquad $ 状态-动作值函数是指初始状态为s并进行动作a，然后执行策略π得到的期望总回报。"""
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./六TL与DRL/图片27.png', caption='图27 ')
    """$ \qquad $ Q函数的贝尔曼方程"""
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./六TL与DRL/图片28.png', caption='图28 ')
    """- 最优策略:"""
    """$ \qquad $ 最优策略：存在一个最优的策略π∗ ，其在所有状态上的期望回报最大。"""
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./六TL与DRL/图片29.png', caption='图29 ')
    """$ \qquad \qquad $ 难以实现"""
    """$ \qquad $ 策略改进："""
    ...
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./六TL与DRL/图片30.png', caption='图30 ')
    """- 值迭代:"""
    """$ \qquad $ 值迭代方法将策略评估和策略改进两个过程合并，来直接计算出最优策略。"""
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./六TL与DRL/图片31.png', caption='图31 ')
    ...
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./六TL与DRL/图片32.png', caption='图32 ')
    ...
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./六TL与DRL/图片39.png', caption='图39 ')
    """- DQN in Atari :"""
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./六TL与DRL/图片40.png', caption='图40 Mnih, Volodymyr, et al. "Human-level control through deep reinforcement learning." Nature 518.7540 (2015): 529-533.')
    """- DQN in Atari :  Human Level Control:"""
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./六TL与DRL/图片41.png', caption='图41 Mnih, Volodymyr, et al. 2015.')
    ...
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./六TL与DRL/图片42.png', caption='图42 ')
    """- 策略梯度（Policy Gradient）:"""
    """$ \qquad $ 策略搜索是通过寻找参数$\\theta$使得目标函数$J(\\theta)$最大。"""
    """$ \qquad $ 梯度上升："""
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./六TL与DRL/图片43.png', caption='图43 ')
    """- 进一步分解:"""
    """$ \qquad $ 参数$\\tau$优化的方向是使得总回报$G(\\tau)$越大的轨迹$\\tau$的概率$p_{\\theta}(\\tau)$也越大。"""
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./六TL与DRL/图片44.png', caption='图44 ')
    """$ \qquad $ $\\frac{\partial{\\theta{logp_{\\theta}{\\tau}}}}{\partial{\\theta}}$是和状态转移概率无关，只和策略函数相关。"""
    """- 策略梯度:"""
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./六TL与DRL/图片45.png', caption='图45 ')
    """- REINFORCE算法:"""
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./六TL与DRL/图片46.png', caption='图46 ')
    """- 如何减少方差？"""
    """$ \qquad $ 训练方差"""
    ...
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./六TL与DRL/图片47.png', caption='图47 ')
    """- 引入基线:"""
    """$ \qquad $ 在每个时刻t，其策略梯度为"""
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./六TL与DRL/图片48.png', caption='图48 ')
    """$ \qquad $ 为了减小策略梯度的方差，引入一个和$𝑎_𝑡$无关的基准函数$𝑏(𝑠_𝑡)$:"""
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./六TL与DRL/图片49.png', caption='图49 ')
    """- 如何选择$𝑏(𝑠_𝑡)$ ？"""
    """$ \qquad $ 为了可以有效地减小方差， $𝑏(𝑠_𝑡)$和$𝐺(\\tau_(𝑡:𝑇))$越相关越好，一个很自然的选择是令$𝑏(𝑠_𝑡)$为值函数$𝑉^{\pi_{\\theta}}(𝑠_t)$."""
    """$ \qquad $ 一个可学习的函数$𝑉_{\phi}(𝑠_𝑡)$来近似值函数。"""
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./六TL与DRL/图片50.png', caption='图50 ')
    """$ \qquad $ 策略函数参数θ的梯度为:"""
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./六TL与DRL/图片51.png', caption='图51 ')
    """- 带基准线的REINFORCE算法:"""
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./六TL与DRL/图片52.png', caption='图52 ')
    """- Actor-Critic算法:"""
    ...
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./六TL与DRL/图片53.png', caption='图53 ')
    """- 不同强化学习算法之间的关系:"""
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./六TL与DRL/图片54.png', caption='图54 ')
    """- 汇总:"""
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./六TL与DRL/图片55.png', caption='图55 ')
...
with tab2:
    ...
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./六TL与DRL/图片4.jpg', caption='图4')
    ...
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./六TL与DRL/图片5.webp', caption='图5')
    ...
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./六TL与DRL/图片6.webp', caption='图6')
    ...
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./六TL与DRL./六TL与DRL/图片7.webp', caption='图7 Monte Carlo （蒙特卡洛）树搜索算法。')
    ...
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./六TL与DRL/图片8.webp', caption='图8 MCTS“插件”打开或关闭时，AlphaGo的实力如何变化的。图为 Silver提供。')
...
with tab3:
    ...
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./六TL与DRL/图片9.webp', caption='图9 人类中介体复合物一直是结构生物学家难以理解的一种多蛋白系统。来源：Yuan He')
    ...
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./六TL与DRL/图片10.webp', caption='图10 来源：EMBL–EBI和https://swissmodel.expasy.org/repository')
    """**问题概述**:"""
    ...
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./六TL与DRL/图片11.webp', caption='图11 AlphaFold算法框架')
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./六TL与DRL/图片12.webp', caption='图12 黄色代表特征抽取，绿色代表结构预测，红色代表potential construction，蓝色代表结构生成')
...
with tab4:
    ...
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./六TL与DRL/图片13.webp', caption='图13 ')
