...
with tab1:
    ...
    with col1:
        st.image('./三卷积CNN/图片25.png', caption='图25 ResNet在ILSVRC和COCO 2015上的战绩')
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./三卷积CNN/图片26.png', caption='图26 ImageNet分类Top-5误差')
    ...
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./三卷积CNN/图片27.png', caption='图27 20层与56层网络在CIFAR-10上的误差')
    """$ \qquad $ 2.残差学习单元： """
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./三卷积CNN/图片28.png', caption='图28 残差学习单元')
    """$ \qquad $ 3.不同的残差单元："""
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./三卷积CNN/图片29.png', caption='图29 不同的残差单元')
    """$ \qquad $ 4.ResNet的网络结构："""
    _, col1 = st.columns([1, 2])
    with col1:
        st.image('./三卷积CNN/图片30.png', caption='图30 ResNet的网络结构')
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./三卷积CNN/图片31.png', caption='图31 18-layer和34-layer的网络效果')
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./三卷积CNN/图片32.png', caption='图32 改进后的残差单元及效果')
...
with tab2:
    ...
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./三卷积CNN/图片33.png', caption='图33')
    r"""- 检测识别的方法："""
    r"""$ \qquad $ 1.传统方法：Hog/SIFT视觉特征提取+svm分类。"""
    r"""$ \qquad $ 2.基于深度学习的目标检测识别方法: one-stage、two-stage。"""
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./三卷积CNN/图片34.jpg', caption='图34 Two-stage算法')
    ...
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./三卷积CNN/图片35.jpg', caption='图35')
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./三卷积CNN/图片36.jpg', caption='图36')
    ...
    _, col1 = st.columns([1, 2])
    with col1:
        st.image('./三卷积CNN/图片37.png', caption='图37 Faster RCNN基本结构（4个）')
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./三卷积CNN/图片38.png', caption='图38 VGG16模型中的faster_rcnn_test.pt的网络结构')
    """- 检测识别的应用————2.YoLo系列："""
    r"""$ \qquad $ YoLo系列是one-stage算法，即直接从图片生成位置和类别，没有明显的生成候选框的过程。YOLO是You only look once的缩写。"""
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./三卷积CNN/图片39.jpg', caption='图39')
    r"""$ \qquad $ YoLo思想：YoLo固定维度的办法是把模型的输出划分成网格形状，每个网格中的cell都可以输出物体的类别和bounding box的坐标，如下图：
"""
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./三卷积CNN/图片40.png', caption='图40')
    r"""$ \qquad $ YoLo核心思想: """
    ...
    _, col1 = st.columns([1, 2])
    with col1:
        st.image('./三卷积CNN/图片41.png', caption='图41')
    r"""$ \qquad $ YoLo模型: """
    _, col1, _ = st.columns([1, 2, 1])
    with col1:
        st.image('./三卷积CNN/图片42.png', caption='图42')
    r"""$ \qquad $ YoLo模型架构:"""
    _, col1, _ = st.columns([1, 2, 1])
    with col1:
        st.image('./三卷积CNN/图片43.png', caption='图43')
...
基本上计算机视觉的技术都会有涉及。 2D的分割，分类在3D中也会用到。"""
    ...
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./三卷积CNN/图片54.png', caption='图54')

    """$ \qquad $ 3D重建："""
    """$ \qquad \qquad $ 根据SFM/SLAM计算的位姿，进行稠密重建恢复场景几何信息。"""
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./三卷积CNN/图片55.png', caption='图55')
    ...
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./三卷积CNN/图片56.png', caption='图56')
    ...
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./三卷积CNN/图片66.png', caption='图66')
...
\end{equation}
    ...
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./三卷积CNN/图片67.png', caption='图67')
    ...
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./三卷积CNN/图片68.png', caption='图68')
    ...
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./三卷积CNN/图片69.gif', caption='图69')

    """- 三维重建的应用场景：比如增强现实（AR）、混合现实（MR)、机器人导航、自动驾驶等领域的核心技术之一。"""
    r"""$ \qquad $ 影像娱乐："""
    _, col1, _ = st.columns([1, 2, 1])
    with col1:
        st.image('./三卷积CNN/图片44.png', caption='图44')
    r"""$ \qquad $ 智能家居："""
    _, col1, _ = st.columns([1, 2, 1])
    with col1:
        st.image('./三卷积CNN/图片45.png', caption='图45')
    r"""$ \qquad $ 文物重建、AR旅游"""
    _, col1, _ = st.columns([1, 2, 1])
    with col1:
        st.image('./三卷积CNN/图片46.png', caption='图46')
    r"""$ \qquad $ 自动驾驶"""
    r"""$ \qquad \qquad $ 在自动驾驶领域的主要应用是高精地图的构建，此应用对于自动驾驶的算法迭代优化、测试都非常重要。"""
    _, col1, _ = st.columns([1, 2, 1])
    with col1:
        st.image('./三卷积CNN/图片47.png', caption='图47')
    r"""$ \qquad $ 大型场景的构建"""
    _, col1, _ = st.columns([1, 2, 1])
    with col1:
        st.image('./三卷积CNN/图片48.png', caption='图48')
    r"""$ \qquad $ 逆向工程"""
    _, col1, _ = st.columns([1, 2, 1])
    with col1:
        st.image('./三卷积CNN/图片49.png', caption='图49')
    r"""$ \qquad $ 机器人、导航"""
    _, col1, _ = st.columns([1, 2, 1])
    with col1:
        st.image('./三卷积CNN/图片50.png', caption='图50')
...
with tab4:
    ...
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./三卷积CNN/图片70.png', caption='图70')
    """$ \qquad $ 阈值分割方法的优缺点："""
...
接下来介绍几个非常著名的分割模型："""
    ...
    with col1:
        st.image('./三卷积CNN/图片60.png', caption='图60')
...
with tab5:
    ...
    _, col1, _ = st.columns([1, 12, 1])
    with col1:
        st.image('./三卷积CNN/图片61.png', caption='图61')
    """- 图像超分辨率："""
    ...
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./三卷积CNN/图片71.png', caption='图71')
...
首先训练HR到LR的GAN，然后使用未配对的LR-HR图像来学习退化，通过应用两阶段过程，模型有效提高了实际LR图像超分辨率重建图像的质量。"""
    ...
    _, col1, _ = st.columns([1, 12, 1])
    with col1:
        st.image('./三卷积CNN/图片62.png', caption='图62')
    """$ \qquad $ SRCNN是end-to-end(端到端)的超分算法，所以在实际应用中不需要任何人工干预或者多阶段的计算，其网络图如下："""
    _, col1, _ = st.columns([1, 12, 1])
    with col1:
        st.image('./三卷积CNN/图片63.png', caption='图63')
...
with tab6:
    ...
    _, col1, _ = st.columns([1, 12, 1])
    with col1:
        st.image('./三卷积CNN/图片64.png', caption='图64')
    ...
    _, col1, _ = st.columns([1, 12, 1])
    with col1:
        st.image('./三卷积CNN/图片65.png', caption='图65')
