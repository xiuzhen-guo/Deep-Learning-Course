...
"""
    ...
    with col1:
        st.image('./二MLP/图片5.jpg', caption='图5')
...
"""
    ...
    with col1:
        st.image('./二MLP/图片7.png', caption='图7')
...
H是系统的预测函数，也称为假设。当系统受到一个实例的特征向量$ X^{(i)} $，就会输出这个实例的一个预测值$ \tilde{y}=h(X^{(i)}) $。"""
    ...
    _, col1, _ = st.columns([1, 2, 1])
    with col1:
        st.image('./二MLP/图片8.png', caption='图8')
    """- 过拟合："""
    r"""$ \qquad $ 训练误差和测试误差的差距太大，泛化性能不好。"""
    _, col1, _ = st.columns([1, 2, 1])
    with col1:
        st.image('./二MLP/图片9.png', caption='图9')
