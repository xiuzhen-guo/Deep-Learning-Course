...
with col1:
    st.image('./四NLP/图片22.png', caption='图22')
"""- Transformer本质上是一个Encoder-Decoder的结构，那么图22可以表示为图23的结构："""
_, col1, _ = st.columns([1, 4, 1])
with col1:
    st.image('./四NLP/图片23.png', caption='图23')
"""- Transformer的Encoder和Decoder均由6个block堆叠而成。"""
col1, col2 = st.columns([1, 1])
with col1:
    st.image('./四NLP/图片24.png', caption='图24')
with col2:
    st.image('./四NLP/图片25.png', caption='图25')
...
_, col1, _ = st.columns([1, 4, 1])
with col1:
    st.image('./四NLP/图片26.png', caption='图26')
"""$ \qquad $ 输入编码作为一个tensor输入到encoder中"""
_, col1, _ = st.columns([1, 4, 1])
with col1:
    st.image('./四NLP/图片27.png', caption='图27')
...
_, col1, _ = st.columns([1, 4, 1])
with col1:
    st.image('./四NLP/图片28.png', caption='图28 Q,K,V的计算示例图')
col1, col2 = st.columns([1, 1])
with col1:
    st.image('./四NLP/图片29.png', caption='图29 Self-Attention计算示例图')
with col2:
    st.image('./四NLP/图片30.png', caption='图30 Q,K,V的矩阵表示')
_, col1, _ = st.columns([1, 4, 1])
with col1:
    st.image('./四NLP/图片31.png', caption='图31 Self-Attention的矩阵表示')
st.latex(r"""Attention(Q,K,V)=softmax(\frac{QK^T}{\sqrt{d_k}})V""")
