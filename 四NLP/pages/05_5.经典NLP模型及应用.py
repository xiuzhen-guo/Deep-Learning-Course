...
with tab1:
    ...
    with col1:
        st.image('./四NLP/图片38.png', caption='图38')
    ...
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./四NLP/图片39.png', caption='图39')
with tab2:
    ...
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./四NLP/图片40.png', caption='图40')
...
with tab3:
    ...
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./四NLP/图片41.png', caption='图41')
    ...
    with col1:
        st.image('./四NLP/图片42.png', caption='图42')
    _, col1, _ = st.columns([1, 2, 1])
    with col1:
        st.image('./四NLP/图片43.png', caption='图43')
    ...
    _, col1, _ = st.columns([1, 1, 1])
    with col1:
        st.image('./四NLP/图片97.png', caption='图97 GPT-2学习效果图')

    """- GPT-3 的模型所采用的数据量之大，高达上万亿，模型参数量也十分巨大，学习之复杂。如图所示："""
    _, col1, _ = st.columns([1, 1, 1])
    with col1:
        st.image('./四NLP/图片98.png', caption='图98')
    ...
    _, col1, _ = st.columns([1, 1, 1])
    with col1:
        st.image('./四NLP/图片99.png', caption='图99')
...
with tab4:
    ...
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./四NLP/图片44.gif', caption='图44')
    """$ \qquad $ 当代表句子终止的符号被模型选择出来之后，停止迭代过程，并进行反符号化处理，得到自然语句译文。"""
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./四NLP/图片45.gif', caption='图45')

    """- 语音识别："""
    """$ \qquad $ 语音识别的目的是将人类语音中的词汇内容转换为计算机可读的输入内容。"""
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./四NLP/图片46.png', caption='图46')
