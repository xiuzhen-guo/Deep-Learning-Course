...
with col1:
    st.image('./四NLP/图片9.jpg', caption='图9 深层循环神经网络')
r"""- 前向传播与随时间反向传播:"""
_, col1, _ = st.columns([1, 4, 1])
with col1:
    st.image('./四NLP/图片10.jpg', caption='图10 RNN沿时间展开后的结构图')
r"""$ \qquad $ RNN前向传播:"""
_, col1, _ = st.columns([1, 4, 1])
with col1:
    st.image('./四NLP/图片11.jpg', caption='图11 RNN前向传播的计算过程')
r"""$ \qquad $ RNN反向传播:"""
_, col1, _ = st.columns([1, 4, 1])
with col1:
    st.image('./四NLP/图片12.jpg', caption='图12 RNN的BPTT计算示意图')
...
_, col1, _ = st.columns([1, 4, 1])
with col1:
    st.image('./四NLP/图片13.gif', caption='图13')
...
_, col1, _ = st.columns([1, 4, 1])
with col1:
    st.image('./四NLP/图片14.gif', caption='图14')
_, col1, _ = st.columns([1, 4, 1])
with col1:
    st.image('./四NLP/图片15.gif', caption='图15')
r"""$ \qquad $ 以此类推，前面所有的输入都对未来的输出产生了影响，可以看到圆形隐藏层中包含了前面所有的颜色。如下图所示："""
r"""$ \qquad $ 当判断意图的时候，只需要最后一层的输出05，如下图所示："""
_, col1, _ = st.columns([1, 4, 1])
with col1:
    st.image('./四NLP/图片16.gif', caption='图16')
_, col1, _ = st.columns([1, 2, 1])
with col1:
    st.image('./四NLP/图片17.gif', caption='图17')
r"""$ \qquad $ RNN的缺点也比较明显："""
_, col1, _ = st.columns([1, 2, 1])
with col1:
    st.image('./四NLP/图片18.jpg', caption='图18')
