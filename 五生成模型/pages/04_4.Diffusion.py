...
with tab1:
    ...
    with col1:
        st.image('./五生成模型/图片118.jpg', caption='图118 生成模型分类')
    ...
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./五生成模型/图片119.png', caption='图119')
    ...
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./五生成模型/图片120.png', caption='图120')
    ...
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./五生成模型/图片121.png', caption='图121')
    ...
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./五生成模型/图片122.png', caption='图122')
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./五生成模型/图片123.png', caption='图123')
    ...
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./五生成模型/图片124.png', caption='图124')
    ...
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./五生成模型/图片125.png', caption='图125')
    ...
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./五生成模型/图片126.png', caption='图126')
    ...
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./五生成模型/图片127.png', caption='图127')
    ...
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./五生成模型/图片128.png', caption='图128')
    ...
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./五生成模型/图片129.png', caption='图129')
    ...
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./五生成模型/图片130.png', caption='图130')
    ...
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./五生成模型/图片131.png', caption='图131')
    ...
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./五生成模型/图片132.png', caption='图132')
    ...
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./五生成模型/图片133.png', caption='图133')
    ...
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./五生成模型/图片134.png', caption='图134')
    ...
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./五生成模型/图片135.png', caption='图135')
...
with tab2:
    ...
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./五生成模型/图片83.png', caption='图83')
    """$ \qquad $ 可以得到条件分布"""
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./五生成模型/图片84.png', caption='图84')
    """$ \qquad $ 接下来，推导可得"""
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./五生成模型/图片85.png', caption='图85')
    col1, col2 = st.columns([1, 1])
    with col1:
        st.image('./五生成模型/图片86.png', caption='图86')
    with col2:
        st.image('./五生成模型/图片87.png', caption='图87')
    """$ \qquad $ 其中,"""
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./五生成模型/图片88.png', caption='图88')
    """$ \qquad $ 如果写成高斯分布的形式,可以得到"""
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./五生成模型/图片89.png', caption='图89')
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./五生成模型/图片90.png', caption='图90')
    """$ \qquad $ 的每一项都可以写成关于的函数，此时可以通过从中采样x来实现损失函数的计算。"""

with tab3:
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./五生成模型/图片91.png', caption='图91')
    """$ \qquad $ 其中$L_T$因为不带有任何科学系的参数，是常数可以忽略不计。"""
    """$ \qquad $ 对于$L_i,i=1,...,T-1$，这一损失函数的形式过于复杂，首先改变$p_{\\theta}(x_{t-1}|x_t)$的建模方式,变成:"""
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./五生成模型/图片92.png', caption='图92')
    """$ \qquad $ 因为对于两个多维高斯来说KL散度为:"""
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./五生成模型/图片93.png', caption='图93')
    """$ \qquad $ 令$\sum_{\\theta}=\sigma_t^2I$,我们可以简化损失函数为"""
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./五生成模型/图片94.png', caption='图94')
    """$ \qquad $ 从而得到"""
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./五生成模型/图片95.png', caption='图95')
    """$ \qquad $ 最后训练过程变成了:"""
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./五生成模型/图片96.png', caption='图96')
