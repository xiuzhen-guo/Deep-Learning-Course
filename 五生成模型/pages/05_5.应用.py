...
with tab1:
    ...
    with col1:
        st.image('./五生成模型/图片103.png', caption='图103')
    """-  主要用的库："""
...
with tab2:
    ...
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./五生成模型/图片104.png', caption='图104')
    """- 贡献：实时monocular面部再现。"""
    """- 面部再现的意义：它是各种应用的基础，如在视频会议中，源视频可以被调整为匹配翻译的面部动作，或者面部视频可以配音成外语等等。"""
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./五生成模型/图片105.png', caption='图105 方法概述')
    """- 1.密集、全局和非刚性的基于模型的绑定"""
    ...
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./五生成模型/图片106.png', caption='图106')
    ...
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./五生成模型/图片107.png', caption='图107')
    ...
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./五生成模型/图片108.png', caption='图108')
    ...
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./五生成模型/图片109.png', caption='图109')
    ...
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./五生成模型/图片110.png', caption='图110')
    ...
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./五生成模型/图片111.png', caption='图111')
    """ 可以重写为标准的最小二乘形式："""
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./五生成模型/图片112.png', caption='图112')
    ...
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./五生成模型/图片113.png', caption='图113')
    """$ \qquad $ 1.相似性度量"""
    ...
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./五生成模型/图片114.png', caption='图114')
    """$ \qquad $ 第一项Dp度量参数空间中的距离："""
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./五生成模型/图片115.png', caption='图115')
    """$ \qquad $ 第二项$D_m$测量稀疏面部landmarks的差异相容性："""
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./五生成模型/图片116.png', caption='图116')
    """$ \qquad $ 在这里，$\Omega$是一组预定义的landmark对，定义了诸如上唇和下唇之间或口腔左右角之间的距离。最后一项$D_a$是一个外观测量项，由以下两部分组成："""
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./五生成模型/图片117.png', caption='图117')
