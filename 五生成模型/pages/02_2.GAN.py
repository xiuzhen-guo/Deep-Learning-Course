...
with tab1:
    ...
    with col1:
        st.image('./五生成模型/图片10.jpg', caption='图10')
    """- GAN的优缺点:"""
    """$ \qquad $ 1.优点："""
...
with tab2:
    ...
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./五生成模型/图片11.png', caption='图11')
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./五生成模型/图片12.jpg', caption='图12')
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./五生成模型/图片13.png', caption='图13')
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./五生成模型/图片14.png', caption='图14')
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./五生成模型/图片15.jpg', caption='图15')
    """- Deep Convolutional GAN:"""
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./五生成模型/图片16.png', caption='图16')
    """$ \qquad $ DCGAN 的判别器和生成器都使用了卷积神经网络（CNN）来替代GAN 中的多层感知机，同时为了使整个网络可微，拿掉了CNN 中的池化层，另外将全连接层以全局池化层替代以减轻计算量。"""
    """- Info-GAN:"""
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./五生成模型/图片17.png', caption='图17')
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./五生成模型/图片18.png', caption='图18')
    ...
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./五生成模型/图片97.jpg', caption='图97')
    ...
    with col1:
        st.image('./五生成模型/图片19.png', caption='图19')
    with col2:
        st.image('./五生成模型/图片20.png', caption='图20')
    """- 损失函数:"""
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./五生成模型/图片21.png', caption='图21')
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./五生成模型/图片22.png', caption='图22')
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./五生成模型/图片23.png', caption='图23')
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./五生成模型/图片24.png', caption='图24')
    """- GAN模型分析:"""
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./五生成模型/图片25.png', caption='图25')
    """$ \qquad $ 假设$𝑝_𝑟(𝑥)，𝑝_{\thta}(𝑥)$已知，则最优的判别器为:"""
    _, col1, _ = st.columns([1, 1, 1])
    with col1:
        st.image('./五生成模型/图片26.png', caption='图26')
    """$ \qquad $ 目标函数变为："""
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./五生成模型/图片27.png', caption='图27')
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./五生成模型/图片28.png', caption='图28')
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./五生成模型/图片29.png', caption='图29')
    """- 不稳定性：生成网络的梯度消失"""
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./五生成模型/图片30.png', caption='图30')
    _, col1, _ = st.columns([1, 1, 1])
    with col1:
        st.image('./五生成模型/图片31.png', caption='图31')
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./五生成模型/图片32.png', caption='图32')
    ...
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./五生成模型/图片33.png', caption='图33')
    ...
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./五生成模型/图片34.png', caption='图34')

    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./五生成模型/图片35.png', caption='图35')
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./五生成模型/图片36.png', caption='图36')
    """- 模型坍塌：生成网络的“错误”目标"""
    """$ \qquad $ 生成网络的目标函数。"""
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./五生成模型/图片37.png', caption='图37')
    """$ \qquad $ 其中后两项和生成网络无关，因此"""
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./五生成模型/图片38.png', caption='图38')
    """- 前向散度和逆向KL散度:"""
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./五生成模型/图片39.png', caption='图39')
    _,col1, col2 = st.columns([1, 1, 1])
    with col1:
        st.image('./五生成模型/图片40.png', caption='图40')
    with col1:
        st.image('./五生成模型/图片41.png', caption='图41')
    ...
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./五生成模型/图片98.png', caption='图98')
    ...
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./五生成模型/图片42.png', caption='图42')
