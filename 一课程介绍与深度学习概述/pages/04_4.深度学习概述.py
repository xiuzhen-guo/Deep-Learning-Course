...
with tab1:
    ...
    with col1:
        st.image('./一课程介绍与深度学习概述/图片9.png', caption='图9')
    """- AI三次发展浪潮："""
...
"""
    ...
    with col1:
        st.image('./一课程介绍与深度学习概述/图片10.png', caption='图10')
    """- AI的两种划分："""
...
"""
    ...
    with col1:
        st.image('./一课程介绍与深度学习概述/图片11.png', caption='图11')
    """- AI发展脉络："""
    _, col1, _ = st.columns([1, 16, 1])
    with col1:
        st.image('./一课程介绍与深度学习概述/图片12.png', caption='图12')
...
"""
    ...
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./一课程介绍与深度学习概述/图片13.png', caption='图13')

    """- 基本过程："""
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./一课程介绍与深度学习概述/图片14.png', caption='图14')
...
"""
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./一课程介绍与深度学习概述/图片15.png', caption='图15')
    """- 典型的ANN结构："""
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./一课程介绍与深度学习概述/图片16.png', caption='图16')
...
"""
    ...
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./一课程介绍与深度学习概述/图片17.png', caption='图17')
    """- 前向网络："""
    ...
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./一课程介绍与深度学习概述/图片18.png', caption='图18 CNN')
    ...
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./一课程介绍与深度学习概述/图片19.png', caption='图19 Auto-Encoder')
    """- 自循环神经网络："""
    r"""$ \qquad $ 自循环神经网络模型，或者序列模型不是因为深度学习才有的，而是很早以前就有相关研究，例如有向图模型中的隐马尔科夫HMM、无向图模型中的条件随机场模型CRF等都是非常成功的序列模型。
"""
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./一课程介绍与深度学习概述/图片20.png', caption='图20 RNN模型')
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./一课程介绍与深度学习概述/图片21.png', caption='图21 GAN')
    """- 增强学习(RL)："""
    ...
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./一课程介绍与深度学习概述/图片22.png', caption='图22 RL代表算法')
...
with tab4:
    ...
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./一课程介绍与深度学习概述/图片23.png', caption='图23 新冠肺炎肺部CT图像智能诊断')
    r"""$ \qquad $ 2.医疗机器人：机器人、人机交互"""
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./一课程介绍与深度学习概述/图片24.jpg', caption='图24 假肢机器人')

    r"""$ \qquad $ 3.远程诊断：知识图谱、深度学习算法"""
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./一课程介绍与深度学习概述/图片25.jpg', caption='图25 远程脑起搏器成功实施，5G医疗落地')

    r"""$ \qquad $ 4.电子病历：自然语言处理、语音识别"""
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./一课程介绍与深度学习概述/图片26.jpeg', caption='图26 深度学习分析电子病历，预测患者病情发展')

    """- 智慧城市："""
    r"""$ \qquad $ 1.智慧物流及建筑服务系统：机器学习、物联网、安全与隐私"""
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./一课程介绍与深度学习概述/图片27.png', caption='图27 某物流公司的物流机器人正在高效工作')

    r"""$ \qquad $ 2.自动驾驶：机器学习、计算机视觉、人机交互"""
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./一课程介绍与深度学习概述/图片28.jpeg', caption='图28 自动驾驶汽车：通过电脑系统实现的无人驾驶的智能汽车')

    r"""$ \qquad $ 3.智能家居：自然语言处理、自然语言理解、安全与隐私、人机交互、语音识别、计算机视觉"""
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./一课程介绍与深度学习概述/图片29.jpg', caption='图29 人用平板电脑控制智能家居助手作为虚拟屏幕 ')

    r"""$ \qquad $ 4.智能交通系统：机器学习、数据挖掘、计算机视觉、自家语言处理、自然语言理解、图像识别、智能芯片"""
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./一课程介绍与深度学习概述/图片30.jpg', caption='图30 智能交通 ')

with tab5:
    """- 研究趋势："""
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./一课程介绍与深度学习概述/图片31.png', caption='图31')
